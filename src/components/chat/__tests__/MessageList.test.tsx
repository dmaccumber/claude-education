...
function textMessage(id: string, role: UIMessage["role"], text: string): UIMessage {
  return makeMessage({ id, role, parts: [{ type: "text", text }] });
}
...
test("MessageList renders user messages", () => {
  const messages = [textMessage("1", "user", "Create a button component")];
...
test("MessageList renders assistant messages", () => {
  const messages = [textMessage("1", "assistant", "I'll help you create a button component.")];
...
test("MessageList shows loading state for last assistant message without content", () => {
  const messages: UIMessage[] = [
    makeMessage({ id: "1", role: "assistant", parts: [] }),
  ];
...
test("MessageList doesn't show loading state for non-last messages", () => {
  const messages: UIMessage[] = [
    textMessage("1", "assistant", "First response"),
    textMessage("2", "user", "Another request"),
  ];
...
test("MessageList renders multiple messages in correct order", () => {
  const messages: UIMessage[] = [
    textMessage("1", "user", "First user message"),
    textMessage("2", "assistant", "First assistant response"),
    textMessage("3", "user", "Second user message"),
    textMessage("4", "assistant", "Second assistant response"),
  ];
...
test("MessageList applies correct styling for user vs assistant messages", () => {
  const messages: UIMessage[] = [
    textMessage("1", "user", "User message"),
    textMessage("2", "assistant", "Assistant message"),
  ];
...
test("MessageList handles empty parts", () => {
  const messages: UIMessage[] = [
    makeMessage({
      id: "1",
      role: "assistant",
